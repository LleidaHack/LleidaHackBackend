...
async def get_all_hacker_geocaching(user_code: str, db: Session = Depends(get_db)):
    return geocaching_service.get_all_hacker_geocaching(db, user_code)


@router.post("/geocaching/hacker/{user_code}/{code}")
async def add_user_geocaching(user_code: str, code: str, db: Session = Depends(get_db)):
