...
def get_user_config(db: Session, userId: int, token: TokenData):
    ...
    userConfig = db.query(ModelUserConfig).filter(
        ModelUserConfig.user_id == userId).first()
...
def update_user_config(db: Session, userId: int,
                       payload: SchemaUserConfigUpdate, data: TokenData):
    userConfig = db.query(ModelUserConfig).filter(
        ModelUserConfig.user_id == userId).first()
    if userConfig is None:
