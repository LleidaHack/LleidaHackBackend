...
def update_tokens(user_id:int, db: Session, access_token:str = None, refresh_token:str = None):
...
def create_access_token(user: ModelUser, db: Session, expires_delta: timedelta = None):
...
def create_refresh_token(user: ModelUser, db:Session, expires_delta: timedelta = None):
    to_encode = {'user_id': user.id, 'email': user.email, 'type': user.type}
    encoded_jwt = jwt.encode(to_encode, SECRET_KEY, algorithm=ALGORITHM)
...
def get_data_from_token(token: str = Depends(oauth2_scheme), refresh: bool = False):
    d = TD()
    if is_service_token(token):
        d.is_admin = True
        d.is_service = True
