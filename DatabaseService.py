...
class DatabaseService():
    ...
    def addUser(self, user: User):
        return self.connector.execute_query("INSERT INTO users (name, password, email, nickname, birthdate, telephone, food_restrictions, shirt_size) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", (user.name, user.password, user.email, user.nickname, user.birthdate, user.telephone, user.food_restrictions, user.shirt_size))
    def updateUser(self, user: User):
        return self.connector.execute_query("UPDATE users SET name = %s, password = %s, email = %s, nickname = %s, birthdate = %s, telephone = %s, food_restrictions = %s, shirt_size = %s WHERE id = %s", (user.name, user.password, user.email, user.nickname, user.birthdate, user.telephone, user.food_restrictions, user.shirt_size, user.id))
