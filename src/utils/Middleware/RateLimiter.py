...
MAIL_ROUTES = {"/v1/auth/reset-password", "/v1/auth/resend-verification", "/v1/auth/contact"}
...
class RateLimitingMiddleware:
    def __init__(self, app, config, secret):
        ...
        self.redis = Redis.from_url(
            ...
            max_connections=50, retry_on_timeout=False, retry=Retry(NoBackoff(), 0),
        )
    ...
    async def consume(self, rule, identity, limit, seconds):
        return int(await self.redis.eval(COUNTER, 1, self.key(rule, identity), limit, seconds))
    ...
    async def __call__(self, scope, receive, send):
        ...
        rules = [("burst", identity, c.burst_per_second, 1),
        ...
        try:
            for rule in rules:
                ...
                if retry:
                    await self.reject(scope, receive, send, 429, "Too many requests. Please try again later.", retry)
                    return
            body = bytearray()
            async def read_body():
                while True:
            ...
            if length and (len(length) > 12 or not length.isdecimal() or int(length) > c.max_body_bytes):
                await self.reject(scope, receive, send, 413, "Request body is too large.")
                return
            try:
            ...
            if login:
                ...
                if len(auth) == 2 and auth[0].lower() == "basic":
                    try:
                        account = base64.b64decode(auth[1], validate=True).decode().split(":", 1)[0]
            ...
            await self.reject(scope, receive, send, 503, "Request protection temporarily unavailable.", 5)
            return
