...
def upgrade():
    ...
    op.create_table('hacker_meal',
                    sa.Column('user_id', sa.Integer(), sa.ForeignKey('hacker.user_id'), primary_key=True, index=True),
                    sa.Column('meal_id', sa.Integer(), sa.ForeignKey('meal.id'), primary_key=True, index=True),
    )
