...
class EventService(BaseService):
    ...
    def get_hacker_group(self, event_id: int, hacker_id: int,
                        data: TokenData):
        event = self.db.query(ModelEvent).filter(ModelEvent.id == event_id).first()
        if event is None:
            raise NotFoundException("Event not found")
        hacker = self.db.query(ModelHacker).filter(ModelHacker.id == hacker_id).first()
        if hacker is None:
    ...
    def update_event(self, id: int, event: EventUpdateSchema,
                    data: TokenData):
        if not data.is_admin:
            if not (data.available and data.type == UserType.LLEIDAHACKER.value):
                raise AuthenticationException("Not authorized")
        db_event = self.db.query(ModelEvent).filter(ModelEvent.id == id).first()
        if db_event is None:
            raise NotFoundException("Event not found")
    ...
    def get_event_groups(self, id: int, data: TokenData):
        event = self.db.query(ModelHackerGroup).filter(ModelHackerGroup.event_id == id)
        if event is None:
            raise NotFoundException("Event not found")
        return event
    ...
    def add_hacker_group(self, id: int, hacker_group_id: int,
                        data: TokenData):
        if not data.is_admin:
            if not (data.available and (data.type == UserType.LLEIDAHACKER.value
    ...
    def remove_hacker_group(self, id: int, hacker_group_id: int, data: TokenData):
        if not data.is_admin:
            if not (data.available and (data.type == UserType.LLEIDAHACKER.value
                                        or data.type == UserType.HACKER.value)):
                raise Exception("Not authorized")
