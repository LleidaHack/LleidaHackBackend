...
class EventService(BaseService):
    ...
    def get_by_id(self, item_id: int) -> Event:
    ...
    def get_hackeps(self, year: int):
        ...
        if e is None:
            raise NotFoundError("We can't find an event for this year or earlier ")

        return e

    def archive(self, event_id: int, data: BaseToken):
        if not data.check([UserType.LLEIDAHACKER]):
            raise AuthenticationError('Not authorized')
        event = self.get_by_id(event_id)
        if event.archived:
            raise InvalidDataError('Event is already archived')
        event.archived = True
        db.session.commit()
        db.session.refresh(event)
        return event

    def unarchive(self, event_id: int, data: BaseToken):
        if not data.check([UserType.LLEIDAHACKER]):
            raise AuthenticationError('Not authorized')
        event = self.get_by_id(event_id)
        if not event.archived:
            raise InvalidDataError('Event is not archived')
    ...
    def get_hacker_group(self, event_id: int, hacker_id: int, data: BaseToken):
        ...
        if hacker not in event.registered_hackers:
            raise NotFoundError('Hacker is not participant')
        user_groups = (
    ...
    def get_event(self, event_id: int, data: BaseToken):
        event = self.get_by_id(event_id)
        if not data.check([UserType.LLEIDAHACKER]):
    ...
    def update_event(self, event_id: int, event: EventUpdate, data: BaseToken):
        if not data.check([UserType.LLEIDAHACKER]):
            raise AuthenticationError('Not authorized')
    ...
    def delete_event(self, event_id: int, data: BaseToken):
        if not data.check([UserType.LLEIDAHACKER]):
            raise AuthenticationError('Not authorized')
        db_event = self.get_by_id(event_id)
    ...
    @BaseService.needs_service(HackerService)
    def is_registered(self, event_id: int, hacker_id: int, data: BaseToken):
        event = self.get_by_id(event_id)
        hacker = self.hacker_service.get_by_id(hacker_id)
        return hacker in event.registered_hackers

    @BaseService.needs_service(HackerService)
    def is_accepted(self, event_id: int, hacker_id: int, data: BaseToken):
        event = self.get_by_id(event_id)
        hacker = self.hacker_service.get_by_id(hacker_id)
        return hacker in event.accepted_hackers

    def has_confirmed(self, event_id: int, hacker_id: int, data: BaseToken):
        user_registration = (
    ...
    @BaseService.needs_service(HackerService)
    def is_participant(self, event_id: int, hacker_id: int, data: BaseToken):
        event = self.get_by_id(event_id)
        hacker = self.hacker_service.get_by_id(hacker_id)
        return hacker in event.participants

    def get_event_meals(self, event_id: int, data: BaseToken):
        event = self.get_by_id(event_id)
        return event.meals

    def get_event_participants(self, event_id: int, data: BaseToken):
        event = self.get_by_id(event_id)
        return event.registered_hackers

    def get_event_sponsors(self, event_id: int):
        event = self.get_by_id(event_id)
        return event.sponsors

    def get_event_groups(self, event_id: int, data: BaseToken):
        event = self.get_by_id(event_id)
        return event.groups

    @BaseService.needs_service(CompanyService)
    def add_company(self, event_id: int, company_id: int, data: BaseToken):
        if not data.check([UserType.LLEIDAHACKER]):
            raise AuthenticationError('Not authorized')
        event = self.get_by_id(event_id)
        if event.archived:
    ...
    def add_hacker(
        ...
        if not data.is_admin:
            if event.max_participants <= len(event.accepted_hackers):
                raise InvalidDataError('Event is full')
        hacker = self.hacker_service.get_by_id(hacker_id)
        if hacker in event.registered_hackers:
            raise InvalidDataError('Hacker already registered')
    ...
    def update_register(
        ...
        hacker = self.hacker_service.get_by_id(hacker_id)
        if hacker not in event.registered_hackers:
            raise InvalidDataError('Hacker is not registered')
        reg = (
    ...
    def add_hacker_group(self, event_id: int, hacker_group_id: int, data: BaseToken):
        ...
        if hacker_group is None:
            raise AuthenticationError('Hacker group not found')
    ...
    @BaseService.needs_service(CompanyService)
    def remove_company(self, event_id: int, company_id: int, data: BaseToken):
        if not data.check([UserType.LLEIDAHACKER]):
            raise AuthenticationError('Not authorized')
        event = self.get_by_id(event_id)
        if event.archived:
        ...
        if company not in event.companies:
            raise InvalidDataError('Company is not sponsor')
    ...
    @BaseService.needs_service('HackerGroupService')
    def remove_hacker_group(self, event_id: int, hacker_group_id: int, data: BaseToken):
        if not data.check([UserType.LLEIDAHACKER, UserType.HACKER]):
            raise AuthenticationError('Not authorized')
        event = self.get_by_id(event_id)
        if event.archived:
    ...
    def get_sizes(self, event_id: int):
        sizes = {}
        event = self.get_by_id(event_id)
    ...
    def get_accepted_and_confirmed(self, event_id: int):
        event = self.get_by_id(event_id)
    ...
    @BaseService.needs_service('HackerGroupService')
    def get_hackers_unregistered(self, event_id: int):
        hackers = self.hacker_service.get_all()
        event = self.get_by_id(event_id)
        return subtract_lists(hackers, event.registered_hackers)

    def count_hackers_unregistered(self, event_id: int):
        return len(self.get_hackers_unregistered(event_id))

    def get_status(self, event_id: int):
        event = self.get_by_id(event_id)
        data = {
            ...
            'acceptedAndConfirmedUsers': len(self.get_accepted_and_confirmed(event_id)),
        }
        for meal in event.meals:
            data[meal.name] = len(meal.users)
        return data

    def get_statistics(self, event_id: int):
        event = self.get_by_id(event_id)
        data = {
            'how_did_you_meet_us': [],
            'location': [],
    ...
    def get_food_restrictions(self, event_id: int):
        event = self.get_by_id(event_id)
    ...
    @BaseService.needs_service(HackerService)
    def get_credits(self, event_id: int, data: BaseToken):
        if not data.check([UserType.LLEIDAHACKER]):
            raise AuthenticationError('Not authorized')
        ...
        regs = (
            db.session.query(HackerRegistration)
            .filter(
                HackerRegistration.event_id == event_id, HackerRegistration.wants_credit
            )
            .all()
        )
    ...
    def confirm_assistance(self, data: AssistenceToken):
        ...
        if user_registration is None:
            raise InvalidDataError('User not registered')
        ...
        if user not in event.accepted_hackers:
            raise InvalidDataError('User not accepted')
        if user_registration.confirmed_assistance:
            raise InvalidDataError('User already confirmed assistance')
    ...
    def force_confirm_assistance(self, user_id: int, event_id: int, data: BaseToken):
        if not data.is_admin:
            raise AuthenticationError("You don't have permissions to do this")
    ...
    def participate_hacker(self, event_id: int, hacker_code: str, data: BaseToken):
        ...
        if hacker in event.participants:
            raise InvalidDataError('Hacker already participating')
        if hacker not in event.accepted_hackers:
            raise InvalidDataError('Hacker not accepted')
        user_registration = (
            db.session.query(HackerRegistration)
            .filter(
        ...
        if not user_registration.confirmed_assistance:
            ...
            # raise InvalidDataError("User not confirmed assitence")
    ...
    def accept_hacker(self, event_id: int, hacker_id: int, data: BaseToken):
        ...
        hacker = self.hacker_service.get_by_id(hacker_id)
        if hacker not in event.registered_hackers:
            raise InvalidDataError('Hacker not registered')
        if hacker in event.accepted_hackers:
            raise InvalidDataError('Hacker already accepted')
        hacker_registration = (
            db.session.query(HackerRegistration)
            .filter(
