...
class AuthenticationService(BaseService):
    ...
    hacker_service = None
    lleida_hacker_service = None
    company_user_service = None
    ...
    def login(self, mail: str, password: str):
        ...
        if not user.is_verified:
            raise InvalidDataError('User not verified')
    ...
    def confirm_reset_password(self, token: ResetPassToken, password: str):
        if token.expt < datetime.now(UTC).isoformat():
            raise InvalidDataError('Token expired')
    ...
    def force_verification(self, user_id: int, data: BaseToken):
        if not data.is_admin:
            raise AuthenticationError("User don'have permissions to do this")
