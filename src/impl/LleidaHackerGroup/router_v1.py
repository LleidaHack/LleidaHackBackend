...
@router.get("/{groupId}", response_model=LleidaHackerGroupGetAll | LleidaHackerGroupGet)
...
@router.post("/")
def add(payload: LleidaHackerGroupCreate, token: BaseToken = Depends(JWTBearer())):
...
def delete_member(
    groupId: int, lleidahackerId: int, token: BaseToken = Depends(JWTBearer())
):
