...
def get_user_configs(db: Session = Depends(get_db),
    ...
    return userConfig_service.get_all_users_config(
        db, token
    )  ##TODO: Faltaria pasarli el token per a que nomes pugui realitzar-ho l'admin


@router.get("/{userId}",
            response_model=Union[UserConfigGetSchema, UserConfigGetAllSchema])
def get_user_config(userId: int,
                    db: Session = Depends(get_db),
                    token=Depends(JWTBearer())):
    return userConfig_service.get_user_config(db, userId,
...
def update_user_config(userId: int,
                       ...
                       db: Session = Depends(get_db),
                       token=Depends(JWTBearer())):
    return userConfig_service.update_user_config(db, userId, payload,
                                                 get_data_from_token(token))
...
def create_user_configs(db: Session = Depends(get_db),
                        token=Depends(JWTBearer())):
    return userConfig_service.create_user_configs(db,
                                                  get_data_from_token(token))
