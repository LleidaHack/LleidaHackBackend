...
def get_all(str: BaseToken | None = Depends(JWTBearer())):
    return lleidahacker_service.get_all()


@router.get("/{userId}", response_model=LleidaHackerGetAll | LleidaHackerGet)
def get(userId: int, data: BaseToken | None = Depends(JWTBearer())):
