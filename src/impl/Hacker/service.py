...
class HackerService(BaseService):
    ...
    def get_hacker(self, hacker_id: int, data: BaseToken):
        user = self.get_by_id(hacker_id)
        if data.check([UserType.LLEIDAHACKER, UserType.HACKER], hacker_id):
    ...
    def remove_hacker(self, hacker_id: int, data: BaseToken):
        ...
        for group in hacker_groups:
            if len(group.members) <= 1:
                ...
                if group.leader_id == hacker_id:
                    members_ids = [h.id for h in group.members]
                    members_ids.remove(hacker_id)
                    group.leader_id = members_ids[0]
        db.session.query(HackerMeal).filter(HackerMeal.user_id == hacker_id).delete()
        ...
        db.session.query(HackerParticipation).filter(
            HackerParticipation.user_id == hacker_id
        ).delete()
        db.session.query(HackerAccepted).filter(
            HackerAccepted.user_id == hacker_id
        ).delete()
    ...
    def update_hacker(self, hacker_id: int, payload: HackerUpdate, data: BaseToken):
        if not data.check([UserType.LLEIDAHACKER]) and not data.check(
            [UserType.HACKER], hacker_id
        ):
            raise AuthenticationError('Not authorized')
    ...
    def ban_hacker(self, hacker_id: int, data: BaseToken):
        if not data.check([UserType.LLEIDAHACKER]):
            raise AuthenticationError('Not authorized')
        hacker = self.get_by_id(hacker_id)
        if hacker.banned:
            raise InvalidDataError('Hacker already banned')
        hacker.banned = True
        db.session.commit()
        db.session.refresh(hacker)
        return hacker

    def unban_hacker(self, hacker_id: int, data: BaseToken):
        if not data.check([UserType.LLEIDAHACKER]):
            raise AuthenticationError('Not authorized')
        hacker = self.get_by_id(hacker_id)
        if not hacker.banned:
            raise InvalidDataError('Hacker already unbanned')
        hacker.banned = False
        db.session.commit()
        db.session.refresh(hacker)
        return hacker
    ...
    def get_hacker_events(self, hacker_id: int):
        hacker = self.get_by_id(hacker_id)
        return hacker.events

    # TODO: #34 Check if token validation is correct
    def get_hacker_groups(self, hacker_id: int):
        hacker = self.get_by_id(hacker_id)
    ...
    #         raise InvalidDataError("Event registration not open")
    #     hacker = self.get_by_id(hacker_id)
    ...
    #         raise InvalidDataError("Event full")
    #     if payload.cv != "" and not isBase64(payload.cv):
    #         raise InvalidDataError("Invalid CV")
