...
@pytest.mark.parametrize("value", ["https://example.test/photo.png", data_uri("image/svg+xml", b'<svg onload="alert(1)"/>'), data_uri("image/png", b"<script>alert(1)</script>"), "data:image/png;base64,@@@"])
...
def test_images_are_decoded_and_reencoded_without_trailing_payload(format, mime):
    ...
    clean = validate_image(data_uri(mime, output.getvalue() + b"<script>bad()</script>"))
...
def test_pdf_is_rebuilt_without_metadata_or_appended_payload():
    ...
    cleaned = validate_cv(data_uri("application/pdf", pdf(writer) + b"<script>bad()</script>"))
...
def test_invalid_pdf_and_oversized_pdf_are_rejected():
    for raw in [b"%PDF-1.7 not actually a document", b"<html>script</html>", b"x" * (MAX_FILE_BYTES + 1)]:
        with pytest.raises(ValueError):
            validate_cv(data_uri("application/pdf", raw))


def test_upload_validation_cannot_be_bypassed_using_api(client, create_user, create_event):
    user = create_user()
    event = create_event([user])
    assert client.put(f"/v1/hacker/{user.id}", headers=user.headers,
                      json={"image": "https://example.test/image.png"}).status_code == 422
    assert client.put(f"/v1/hacker/{user.id}", headers=user.headers,
                      json={"image": data_uri("image/png", png())}).status_code == 200
