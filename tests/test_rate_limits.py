        ...
        try:
            for index, email in enumerate(
                ...
                transport = httpx.ASGITransport(
                    app=app, client=(f"192.0.2.{index}", 123)
                )
                async with httpx.AsyncClient(
                    ...
                    response = await client.get(
                        "/v1/auth/login/", headers={"Authorization": f"Basic {auth}"}
                    )
    ...
    async def check():
        ...
        try:
            async with httpx.AsyncClient(
                ...
                assert (
                    await client.post(
                        "/v1/hacker/signup", json={"email": "person@example.test"}
                    )
                ).status_code == 200
                assert (
                    await client.post(
                        "/v1/auth/reset-password?email=PERSON@example.test"
                    )
                ).status_code == 200
                assert (
                    await client.post(
                        "/v1/auth/resend-verification?email=person@example.test"
                    )
                ).status_code == 429
        finally:
            await app.redis.aclose()
...
def test_body_limits_declared_and_chunked_and_timeout():
    async def check():
        app = middleware(max_body_bytes=1024, body_timeout_seconds=0.01)
...
def test_storage_failure_rejects_before_endpoint():
    async def check():
        ...
        try:
            with patch.object(
                app.redis, "eval", side_effect=ConnectionError("offline")
            ):
                async with httpx.AsyncClient(
                    transport=httpx.ASGITransport(app=app), base_url="http://test"
                ) as client:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                for email in ("one@test", "two@test"):
                    assert (
                        await client.post("/v1/hacker/signup", json={"email": email})
                    ).status_code == 200
                assert (
                    await client.post("/v1/hacker/signup", json={"email": "three@test"})
                ).status_code == 429
            for index in range(2):
                ...
                async with httpx.AsyncClient(
                    ...
                    response = await client.post(
                        "/v1/auth/contact", json={"email": f"other{index}@test"}
                    )
...
def test_ipv6_addresses_share_subnet_quota():
    async def check():
        ...
        try:
            for index in range(2):
                transport = httpx.ASGITransport(
                    app=app, client=(f"2001:db8::{index + 1}", 123)
                )
                async with httpx.AsyncClient(
                    transport=transport, base_url="http://test"
                ) as client:
                    assert (await client.get("/")).status_code == (
                        200 if index == 0 else 429
                    )
        finally:
            await app.redis.aclose()
