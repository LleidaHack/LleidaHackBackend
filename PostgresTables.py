...
PostgresTable.TABLES.append(PostgresTable("assigned_group", "group_id serial,user_id serial,constraint pk_assigned_group primary key(group_id, user_id),constraint fk_assigned_group_id foreign Key(group_id) references llhk_group(id),constraint fk_assigned_group_user_id foreign key(user_id) references llhk_user(id) "))
...
PostgresTable.TABLES.append(PostgresTable("lives_at", "user_id serial,address_id serial,constraint pk_lives_at Primary Key (user_id, address_id),constraint pk_user_lives Foreign Key (user_id) references llhk_user(id),constraint pk_address_lives Foreign key (address_id) references Address(id)"))
