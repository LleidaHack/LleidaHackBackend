...
USER_TABLE = PostgresTable("Users", "id serial,name varchar(30) NOT NULL,nickname varchar(20),email varchar(30),password varchar(50),birthday date NOT NULL,phone_number varchar(20),food_restrictions varchar(300),shirt_size varchar(6),constraint pk_users PRIMARY KEY (id);")
...
LLEIDAHACKER_TABLE = PostgresTable("lleida_hacker", "id serial constraint pk_lleida_hacker Primary Key,NIF varchar(9) unique not null,student boolean,active boolean,image bytea,constraint fk_lleida_hacker Foreign Key(id) References Users(id);")
...
COMPANY_TABLE = PostgresTable("company", "id serial constraint pk_company primary key,phone_number varchar(30),email varchar(100),name varchar(100),nif varchar(100) unique,logo bytea;")
