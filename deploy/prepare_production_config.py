...
def main():
    ...
    if any((dest / name).exists() for name in ["runtime.env", "compose.json", "static"]):
    ...
    environment = {
        "ENV": "main", "LOCAL": "false",
        "FRONT_URL": settings["front_url"], "BACK_URL": settings["back_url"],
        ...
        "RATE_LIMIT__ENABLED": "true", "RATE_LIMIT__REDIS_URL": "redis://redis-main:6379/0",
        "RATE_LIMIT__PREFIX": "lleidahack:main:limits", "FORWARDED_ALLOW_IPS": proxy_ip,
        "WEB_CONCURRENCY": "2",
    }
    ...
    (dest / "runtime.env").write_text("".join(f"{key}={value}\n" for key, value in environment.items()))
    subprocess.run(["docker", "cp", "backend-main:/app/static", str(dest / "static")], check=True)
    ...
    redis_image = next((name for name in redis["RepoDigests"] if name.startswith("redis@sha256:")), None)
    if not redis_image:
    ...
    config = {
        ...
        "services": {
            "backend-main": {
                ...
                "container_name": "backend-main", "restart": "unless-stopped",
                ...
                "volumes": [{"type": "bind", "source": str(dest / "static"), "target": "/app/static", "read_only": True}],
                "networks": ["production"], "logging": logging,
                "healthcheck": {"test": ["CMD", "python", "-c", "import urllib.request; urllib.request.urlopen('http://127.0.0.1:8000/openapi.json', timeout=5)"], "interval": "15s", "timeout": "6s", "retries": 4, "start_period": "20s"},
                "depends_on": {"redis-main": {"condition": "service_healthy"}},
            },
            "redis-main": {
                "image": redis_image, "container_name": "redis-main", "restart": "unless-stopped",
