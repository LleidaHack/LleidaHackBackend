...
def snapshot(connection, baseline=None):
    ...
    with connection.cursor() as cursor:
        ...
        for table in tables:
            ...
            query = sql.SQL(
                'SELECT row_to_json(r)::text FROM (SELECT {} FROM public.{}) r ORDER BY row_to_json(r)::text COLLATE "C"'
            ).format(
        ...
        names = (
            ...
            if baseline is None
            else baseline["sequences"]
        )
        for name in names:
            cursor.execute(
                sql.SQL("SELECT last_value, is_called FROM public.{}").format(
                    sql.Identifier(name)
                )
            )
...
def main():
    ...
    if baseline is None:
        ...
        changes = [
            ...
            if result["tables"][name] != baseline["tables"][name]
        ]
        raise SystemExit(
            f"Data preservation FAILED: changed tables={changes}; sequences_equal={result['sequences'] == baseline['sequences']}"
        )
    print(
        f"{args.mode}: {len(result['tables'])} tables, {sum(t['count'] for t in result['tables'].values())} rows, {len(result['sequences'])} sequences verified"
    )
