...
def user_id_and_token():
    response = client.post("/user/",
                           data=SchemaUser(name="test", nickname=random_string(10), password=random_password(10), birthdate="2000-01-01", food_restrictions="None", email=random_string(5)+"@test.com", telephone="".join([str(random.randint(0,9)) for _ in range(10)]), address="test", shirt_size="M", image=base64.b64encode(b'test').decode('utf-8'), is_image_url=False))
...
def test_get_user_by_email(user_id_and_token):
    user_id, token = user_id_and_token
    response = client.get(f"/user/email/{email}", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
    assert response.json()["email"] == email

def test_get_user_by_nickname(user_id_and_token):
    user_id, token = user_id_and_token
    response = client.get(f"/user/nickname/{nickname}", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
    assert response.json()["nickname"] == nickname

def test_get_user_by_phone(user_id_and_token):
    user_id, token = user_id_and_token
    response = client.get(f"/user/phone/{phone}", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
    assert response.json()["telephone"] == phone

def test_get_user_by_code(user_id_and_token):
    user_id, token = user_id_and_token
    response = client.get(f"/user/code/{code}", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
